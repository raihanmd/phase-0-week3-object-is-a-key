...
function countProfit(shoppers) {
  ...
  const product = listBarang.map((_) => {
    return {
      product: _[0],
      shoppers: [],
      leftOver: _[2],
  ...
  shoppers.map((item) => {
    const productToBuy = product.find((_) => _.product == item.product);
    const productIndex = product.indexOf(productToBuy);

    if (!productToBuy || productToBuy.leftOver < item.amount) return;

    product[productIndex].shoppers.push(item.name);
    product[productIndex].leftOver -= item.amount;
    product[productIndex].totalProfit +=
      listBarang[productIndex][1] * item.amount;
  });
